...
class OrderPlacement:
    ...
    def __init__(self, cart, user_profile, restaurant_menu, restaurant):
        ...
        self.ispreordered = False # bool to reflect has the order been pre ordered
    ...
    def preorder(self, date):
        ...
        if self.paymentDone or date < currentDateTime: #check if order has been payed
            return
    ...
    def edit_preorder_delivery_time(self, new_delivery_date):
        ''' Enables editing preorder delivery date
        '''
        if not self.ispreordered: #if order has not a set delivery time, there is nothing to edit
            return
        
        current_datetime = datetime.datetime.now()
        time_difference = (new_delivery_date - current_datetime).total_seconds() / 3600
        if time_difference < 30:
            return
        
        self.preorder_delivery_time = new_delivery_date

    def valid_preorder_time(self, delivery_date):
        """
        Check if given date is during restaurant operating time 
        Returns true if delivery date is in operating hours.
        """
        return self.restaurant["openingTime"] <= delivery_date.hour < self.restaurant["closingTime"]
...
class TestOrderPlacement(unittest.TestCase):
    ...
    def test_preorder_sets_delivery_date(self):
        ...
        while not self.order.valid_preorder_time(orderDate):
    ...
    def test_cannot_edit_preorder_date_under_thirty_hours_of_preorder_date(self):
        ...
        self.order.preorder(orderDate)
        self.order.edit_preorder_delivery_time(datetime.datetime.now() + datetime.timedelta(minutes=30))
    ...
    def test_cannot_set_preorder_delivery_time_outside_of_restaurant_open_hours(self):
        ...
        while self.order.valid_preorder_time(orderDate):
            orderDate = orderDate + datetime.timedelta(hours=3)
